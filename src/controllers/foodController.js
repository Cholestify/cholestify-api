...
class foodController {
  ...
  static async getFood(request, h) {
    try {
      const dataFoods = await foodService.getAllFoods();
      return h
        .response({
          error: false,
  ...
  static async getFoodRecommendation(request, h) {
    const a = Math.floor(Math.random() * 700) + 1;
    const b = Math.floor(Math.random() * 700) + 1;
    const c = Math.floor(Math.random() * 700) + 1;

    try {
      const dataFoods = await foodService.getAllFoodsRecommendation(a, b, c);
      return h
        .response({
          error: false,
          message: "Get Food successfully",
  ...
  static async updateFood(request, h) {
    ...
    const update = await foodService.updateFood(
      id,
      food,
      caloricValue,
      fat,
